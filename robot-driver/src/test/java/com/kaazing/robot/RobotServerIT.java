...
public class RobotServerIT {
    ...
    @Test(timeout = 2000)
    public void canOverrideFormatToV1() throws Exception {
        // Restart the robot to make sure we have a default parser that is different
        control.close();
        robot.stop();
        robot.start("text/x-robot-2");
        control = new Socket();
        control.connect(new InetSocketAddress("localhost", 61234));
        ...
        out.append("content-type:text/x-robot\n");
        out.append("\n");
        out.append("accept tcp://localhost:62345\n");
        out.append("accepted\n");
        out.append("connected\n");
    ...
    @Test(timeout = 2000)
    public void canOverrideFormatToV1UsingV2ScriptShouldFail() throws Exception {
        // Restart the robot to make sure we have a default parser that is different
        control.close();
        robot.stop();
        robot.start("text/x-robot-2");
